...
function renderOneHikeFull(hike) {
  ...
  image.setAttribute('src', `${imgBasePath}${hike.imgSrc}`);
  image.setAttribute('alt', `${hike.imgAlt}`);
  item.appendChild(image);
  let name = document.createElement('h2');
  name.innerHTML = hike.name;
  item.appendChild(name);
  let distance = document.createElement('div');
  distance.innerHTML = `<h3>Distance</h3><p>${hike.distance}</p>`;
...
list.addEventListener('click', e => {
  ...
  if (e.target.closest('div').className === 'comments') {
    const item = `   `;
  }
});

export function addCommentFormListener() {
...
/*
  <img src="${imgBasePath}${hike.imgSrc}" alt="${hike.imgAlt}">
        ...
        <div id="hike-comment">
            ...
            <textarea id="${hike.name}" name="${hike.name}" rows="4" cols="50"></textarea>
